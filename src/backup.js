...
//works in ms
  ...
  //     return {break_length: prevState.break_length - 60000, start_break: prevState.break_length - 60000 }
  //     })
  //     break;
  ...
  //     this.setState((prevState) => {
  //       return {session_length:prevState.session_length -60000, start:prevState.session_length -60000 }
  //     })
  //     break;
...
// this works in ms
    ...
    // function milisecondConverter(x){
    //   let date = new Date(1969,11,31,16,x,0);
    //   let h =date.getHours();
    //   let m = date.getMinutes();
    //   let s = date.getSeconds();
    //   console.log(date);
  ...
  return (
    <div className="App container-fluid">
      <h1 className="display-1">Jiu Jitsu Round Timer</h1>

      <div id = "break-label" >
      <button id = "break-decrement" className="down btn btn-primary" onClick={() => this.handleClick("break-decrement", 60000)}>Down</button>
      Break Length
      <button id = "break-increment" className="up btn btn-primary" onClick={() => this.handleClick("break-increment", 60000)}>Up</button>
      {/* <p id = "break-length">{milisecondConverterNoSec(this.state.real_break_length)}</p> */}
      <p id = "break-length">{this.state.break_length}</p>
      </div>

      <div id = "session-label">
      <button id = "session-decrement" className="down btn-primary" onClick={() => this.handleClick("session-decrement", 60000)}>Down</button>
      Session Length
      <button id = "session-increment" className="up btn-primary" onClick={()=> this.handleClick("session-increment", 60000)}>Up</button>
      ...
      <p id = "time-left" className="display-4">
      {this.state.running && !this.state.running_break || 
      !this.state.running && !this.state.running_break && this.state.start <= this.state.real_session_length && this.state.start_break == this.state.real_break_length 
      ? milisecondConverter(this.state.start) :milisecondConverter(this.state.start_break)}</p>
      </div>
      ...
      {/* <p>Timer is now {this.state.running ? "True": "False"}</p>
  <p>Break is now {this.state.running_break? "True": "False"}</p>
      <p>Start is : {this.state.start}</p>
      <p>Break is : {this.state.break_length}</p> */}
      {/* <p>poop</p> */}
